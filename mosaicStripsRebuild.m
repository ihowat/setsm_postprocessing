function mosaicStripsRebuild(meta,tiles,res,outdir,projstr,varargin)
% MOSAICSTRIPSREBUILD upper level function for mosaicking DEM strips to a single tile, skips align, merge and tif export steps
%
...
for n=1:length(tiles.I)
    ...
    elseif ~exist(lowresdem,'file')
        fprintf('tile low res dem does not exist %s\n',tiles.I{n})
    else
        fprintf('rebuilding tile %s\n',tiles.I{n})
        m=matfile(lowresdem);
        strips2tileRebuild(m,res);
    end
