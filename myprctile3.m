...
%x = par.Results.x;
%p = par.Results.p;
dim = 3;%par.Results.dim;
method = 'exact';%par.Results.Method;
delta = 1e3; %par.Results.Delta;
...
% the output type. Sometimes P will be data-like and X is table-like, and
% if someone decided to put P on the GPU we want to respect that.
...
elseif isinteger(x)
    ...
    try
        ...
        % For types like duration without double conversion, just proceed
    end
end
