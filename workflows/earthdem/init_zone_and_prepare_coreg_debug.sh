...
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic create-working-dirs "$UTM_ZONE" --verbose
sleep_for_duration


echo "Linking source matfiles"
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic link-source-matfiles "$UTM_ZONE" --verbose
sleep_for_duration
...
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic link-files-to-stage --src "./$UTM_ZONE/00-matfiles" --dst "./$UTM_ZONE/10-coregistration-debug" --src-suffix ".mat" --verbose

echo "Linking fin files to 10-coregistration-debug directory"
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic link-files-to-stage --src "./$UTM_ZONE/00-matfiles" --dst "./$UTM_ZONE/10-coregistration-debug" --src-suffix ".fin" --verbose
sleep_for_duration

echo "Preforming dryrun for coreg-debug stage"
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic coreg-debug "$UTM_ZONE" "./$UTM_ZONE/all_supertiles.txt" --slurm --dryrun

echo "Processing complete. Run the following command to submit to the cluster:"
echo "cd $UTM_ZONE && mamba run -n $MAMBA_ENV_NAME --live-stream earthdem-mosaic coreg-debug $UTM_ZONE ./all_supertiles.txt --slurm"
exit 0