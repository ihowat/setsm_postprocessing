...
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic link-files-to-stage --src 00-matfiles/ --dst 00-matfiles/ --src-suffix _reg_fill.mat --dst-suffix _reg_fill_merge.mat --verbose
sleep_for_duration


echo "Creating neighbor index"
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic create-neighbor-index "$UTM_ZONE"
sleep_for_duration

echo "Preforming dryrun for merge buffers by row"
mamba run -n "$MAMBA_ENV_NAME" --live-stream earthdem-mosaic merge-buffers "$UTM_ZONE" ./all_supertiles.txt row --slurm --dryrun

echo "Processing complete."
echo ""
echo "Run the following command to submit to the cluster:"
echo "    mamba run -n $MAMBA_ENV_NAME --live-stream earthdem-mosaic merge-buffers $UTM_ZONE ./all_supertiles.txt row --slurm"
exit 0