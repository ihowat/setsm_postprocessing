function batch_initializeMosaic_rema(regionName,strt,inc)
% strt=1;
% inc=1;
%paths/files
if ismac
    tileDefFile = '/Users/ihowat/gdrive/projects/rema2/rema_tile_definitions.mat';
       outDir = '/Users/ihowat/project/howat.4/rema_mosaic_v13e';
%    addpath('/Users/ihowat/unity-home/demtools');
else
    tileDefFile = '/home/howat.4/rema2/rema_tile_definitions.mat';
    outDir = '/fs/project/howat.4/howat.4/rema_mosaic_v13e';
 %   addpath('/home/howat.4/demtools');
end

tileDefs=load(tileDefFile);
n = ~cellfun( @ischar, tileDefs.regionName);
tileDefs.regionName(n) = cellfun( @num2str, tileDefs.regionName(n), 'uniformoutput',0);
tileDefs.regionName = strrep(tileDefs.regionName,'region','rema');

IA = contains(tileDefs.regionName,regionName);

tileDefs = structfun( @(x) x(IA), tileDefs, 'uniformoutput',0);

outDir = [outDir,'/',regionName];

if ~exist(outDir,'dir')
    mkdir(outDir)
end

for i=strt:inc:length(tileDefs.I)
    tileName = tileDefs.I{i};
    
    if tileDefs.coastlinePolyshape(i).NumRegions == 0
        fprintf('no land in %s, skipping\n',tileName)
        continue
    end
    
    initializeMosaic('rema',tileName,outDir)
