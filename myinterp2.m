function Vq = myinterp2(X, Y, V, Xq, Yq, method)
%function Vq = myinterp2(varargin)
...
%   monotonic vectors, myinterp2 changes them to a plaid internally.
...
%       Vq = myinterp2(X,Y,V,Xq,Yq); mesh(Xq,Yq,Vq)
%
...
%    error(message('MATLAB:myinterp2:InvalidExtrapval'))
%end
...
%    error(message('MATLAB:myinterp2:nargin'));
%end
...
%        error(message('MATLAB:myinterp2:NtimesInvalid'));
%    end
%    
...
warning(message('MATLAB:myinterp2:NaNstrip'));
if isempty(V) || isvector(V)
    error(message('MATLAB:myinterp2:NotEnoughPointsNanStrip'));
end
...
try
    ...
    if any(strcmp(gime.identifier,{'MATLAB:griddedInterpolant:NotNdGridErrId', ...
        ...
        error(message('MATLAB:myinterp2:InvalidMeshgrid'));
    else
        rethrow(gime);
    end
end
