function qcTile_rema(tileFile,meta)
...
tileschema='V:/pgc/data/scratch/claire/repos/setsm_postprocessing_pgc/PGC_Imagery_Mosaic_Tiles_Antarctic.mat'; %PGC/NGA Tile definition file, required
...
%             f(isnan(m.rmse))=[]; % remove unused data
        ...
        while true
            ...
            % load qc data
            qcFile = [fileparts(fileparts(fileName)),'\qc.mat'];
            ...
            if exist(qcLockFile, 'file') ~= 2
                ...
                if isempty(user_id)
                    ...
                    write_lock();
                elseif strcmp(user_id, USERNAME)
                    if ~strcmp(qcLockFile, QC_LOCK_FILE)
                        fprintf(2, 'Somehow you have multiple qc.mat files under lock. Please make Erik fix this issue.\n');
                        fprintf(2, 'QC_LOCK_FILE: %s\n', QC_LOCK_FILE);
                        fprintf(2, 'qcLockFile: %s\n', qcLockFile);
                        exit
                    end
                else
