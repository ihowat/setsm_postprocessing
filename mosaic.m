function mosaic(db,projstr,tiles,res,outdir,varargin)
% MOSAIC upper level function for mosaicking DEM strips to a set of tiles
%
%  mosaic(db,projstr,tiles,res,outdir)
%  mosaic(db,projstr,tiles,res,outdir,gcp)
...
for n=1:length(tiles.I)
    ...
    %                 delete(outname);
        ...
        strips2tile(db,projstr,tiles.x0(n),tiles.x1(n),tiles.y0(n),tiles.y1(n),res,...
            outname,'disableReg');
    end
