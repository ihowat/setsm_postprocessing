...
get_water_height_erode_px = ceil(150 / dx);
get_water_height_mask = imerode(M, ones(get_water_height_erode_px*2+1));
water_height = nan(size(M));
water_height(get_water_height_mask) = R(get_water_height_mask);
water_height = movmean(movmean(water_height, ceil(1000/dx)*2+1, 1, 'omitnan'), ceil(1000/dx)*2+1, 2, 'omitnan');
water_height(get_water_height_mask) = R(get_water_height_mask);
water_height(water_height > R) = R(water_height > R);
